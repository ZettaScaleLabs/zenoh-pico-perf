...
int main(int argc, char** argv)
{
    ...
    attr.displayName = UA_LOCALIZEDTEXT("en-US", "/test/thr");
    UA_String val = UA_STRING("");
    UA_Variant_setScalar(&attr.value, &val, &UA_TYPES[UA_TYPES_STRING]);

    UA_NodeId newNodeId = UA_NODEID_STRING(1, "/test/thr");
    UA_NodeId parentNodeId = UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER);
    UA_NodeId parentReferenceNodeId = UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES);
    UA_NodeId variableType = UA_NODEID_NULL;
    UA_QualifiedName browseName = UA_QUALIFIEDNAME(1, "/test/thr");
