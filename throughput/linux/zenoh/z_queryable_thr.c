...
{
    if (argc != 3 && argc != 5) {
        printf("USAGE:\n\tz_get_thr <scenario> <payload_size> [<zenoh-locator> <zenoh-mode>]\n\n");
        exit(-1);
    }
    ...
    z_owned_queryable_t qable = z_declare_queryable(z_loan(s), z_keyexpr("test/thr"), z_move(callback), NULL);
    if (!z_check(qable)) {
        return -1;
    }
